...
// BasePattern ???
public class Stage1Pattern6 : BasePattern
{
    ...
    protected override IEnumerator ProcessPattern()
    {
        ...
        yield return new WaitForSeconds(3f); // 공이 느리니까 대기 시간도 넉넉히

        // [Wave 2] 상하에서 엇갈려 나오기 (세로 압박)
        ...
        // [Wave 3] 사방에서 동시에 조여오기 (가운데로 몰기)
        SpawnProjectile(LToR[0], Vector2.right, ProjectileSpeed);
        SpawnProjectile(RToL[3], Vector2.left, ProjectileSpeed);
        SpawnProjectile(UToD[0], Vector2.down, ProjectileSpeed);
        SpawnProjectile(DToU[3], Vector2.up, ProjectileSpeed);

        yield return new WaitForSeconds(1f); // 0.8초 뒤에 나머지 구멍 막기
        ...
        yield return new WaitForSeconds(3.0f); // 마지막 공이 지나갈 때까지 충분히 대기
